...
class NexusStreamer extends EventEmitter {
  constructor(host, cameraUUID, sessionToken) {
    super();
    let self = this;
  ...
  requestStartPlayback() {
    ...
    let request = {
      ...
      profile: StreamProfile.VIDEO_H264_2MBIT_L40,
      other_profiles: [
  ...
  handlePlaybackPacket(payload) {
    ...
    if (packet.channel_id === self.videoChannelID) {
      ...
      self.ffmpeg.stdin.write(Buffer.concat([Buffer.from([0x00, 0x00, 0x00, 0x01]), packet.payload]));
    }
  }
  ...
  setupFFMPEGPipe(request) {
    ...
    if (sessionInfo) {
      ...
      let ffmpegCommand = '-i - -vcodec libx264 -an -pix_fmt yuv420p -r '+ fps +' -f rawvideo -tune zerolatency -vf scale='+ width +':'+ height +' -b:v '+ bitrate +'k -payload_type 99 -ssrc 1 -f rtp -srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params '+videoKey.toString('base64')+' srtp://'+targetAddress+':'+targetVideoPort+'?rtcpport='+targetVideoPort+'&localrtcpport='+targetVideoPort+'&pkt_size=188&buffer_size=65535';
