...
class NexusStreamer extends EventEmitter {
  constructor(host, cameraUUID, sessionToken, config) {
    super();
    let self = this;
  ...
  setupFFMPEGPipe(request) {
    ...
    if (sessionInfo) {
      ...
      let ffmpegCommand = '-i - -vcodec ' + self.ffmpegCodec + ' -an -pix_fmt yuv420p -f rawvideo -x264-params "intra-refresh=1:bframes=0" -vf scale='+ width +':'+ height +' -b:v '+ bitrate +'k -payload_type 99 -ssrc '+ videoSsrc +' -f rtp -srtp_out_suite AES_CM_128_HMAC_SHA1_80 -srtp_out_params '+videoKey.toString('base64')+' srtp://'+targetAddress+':'+targetVideoPort+'?rtcpport='+targetVideoPort+'&localrtcpport='+targetVideoPort+'&pkt_size=188';
